...
function filter (functions, answer, to) {
  Object.keys(functions).forEach((key) => {
    if (!answer[key] && functions[key]["filterFile"]) rm(path.join(to, functions[key]["filterFile"]))
  })
}
...
function compileSub (functions, answer, to) {
  function askAndCompiler (index) {
    ...
    if (answer[key] && functions[key]["prompts"]) ask(functions[key]["prompts"]).then((answer) => {
